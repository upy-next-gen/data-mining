...
def procesar_archivo(archivo_path, output_dir):
    ...
    try:
        ...
        if agregados:
            ...
            logger.error(f"  ✗ No se encontraron registros válidos para procesar")
            return False, stats
...
def main():
    ...
    # Buscar el archivo de exploración más reciente
    exploracion_files = list(Path(".").glob("exploracion_cb_*.json"))
    if exploracion_files:
        latest_exploration = max(exploracion_files, key=lambda p: p.stat().st_mtime)
        logger.info(f"Usando archivo de exploración: {latest_exploration}")
        
        with open(latest_exploration, 'r', encoding='utf-8') as f:
            data = json.load(f)
